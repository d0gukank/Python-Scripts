...
for i in res["scan"]:
	if res["scan"][i]["tcp"][80]["state"] == "open" and res["scan"][i]["tcp"][443]["state"] == "false":
		f.write ("<a target=\"_blank\" href=\"http://"+res["scan"][i]['addresses']['ipv4']+"\">"+res["scan"][i]['addresses']['ipv4']+":80</a>")
		f.write("<br><br>")
	if res["scan"][i]["tcp"][80]["state"] == "false" and res["scan"][i]["tcp"][443]["state"] == "open":
		f.write ("<a target=\"_blank\" href=\"https://"+res["scan"][i]['addresses']['ipv4']+"\">"+res["scan"][i]['addresses']['ipv4']+":443</a>")
		f.write("<br><br>")
